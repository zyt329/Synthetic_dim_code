...
function bond(
    conf::microstate,
    discovered::Array{Tuple{Int64,Int64},1},
    neighb::Tuple{Int64,Int64},
    val::Int64,
    T::Float64,
)
    J = conf.J
    ΔE = 0.0
    for ind in neighbours(conf, neighb)
        if ind ∈ discovered
            ΔE += bond_energy_diff(conf, ind, neighb, val)
        end
    end
    p = 1.0 - exp(min(0.0, -1 / T *  ΔE))
    r = rand()
    if r < p
        return true
    else
