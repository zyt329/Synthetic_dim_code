...
"""
function driver(; L = 8, Q = 16, J1 = [range(0.0, 1.0, length = 5);0.4;0.45], sweeps = 10^6, Temp = range(0.5, 1.0, length = 50))
    Esim = []
    Csim = []
    Bsim = []
    simulation = []
...
L = 8; Q = 16
J1 = range(0.0, 1.0, length = 11);sweeps = 10^6;Temp = range(0.5, 1.0, length = 50)
content = "Max_def_nonperiodic_larger_Temp"
...
#save("Cal_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld", "sim", sim)/nfs/home/zyt329/Research/Synthetic_dim_code/Wolff_test/
#save("Result_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld", "sim", sim)
#=save("E:/UC Davis/Research/Synthetic Dimensions/Synthetic_dim_code/test/test_Metropolis_Max_def_wideTemp_L_$(L)__Q_$(Q)__sweeps_10e6_Date_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld", "sim", sim)
println("L=$L, Q=$Q, Max_def_wideTemp finished at $(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS"))")

save("/nfs/home/zyt329/Research/Synthetic_dim_code/test/test_Metropolis_Max_def_wideTemp_L_$(L)__Q_$(Q)__sweeps_10e6_Date_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld", "sim", sim)
println("L=$L, Q=$Q, Max_def_wideTemp finished at $(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS"))")=#
