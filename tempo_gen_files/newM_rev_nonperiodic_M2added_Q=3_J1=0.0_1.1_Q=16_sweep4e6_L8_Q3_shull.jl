...
L = 8; Q = 3
J1 = range(0.0,1.1,length=11);sweeps = 4*10^6;#critical_region1 = [0.2, 0.4];critical_region2 = [0.55,0.7]
Temp=range(0.01,1.00,length=50)
#=Temp = cat(range(0.01, critical_region1[1]-0.01, length = 10),
range(critical_region1[1], critical_region1[2], length = 10),
range(critical_region1[2]+0.01, critical_region2[1], length = 10),
range(critical_region2[1]+0.01, critical_region2[2], length = 50),
range(critical_region2[2]+0.01, 1, length = 10),
dims=1)=##Temp range determined by fcn in the driver, not here.
content = "newM_nonperiodic_M2added_Q3_J1$(J1[1])_$(J1[end])_Q$(Q)_sweep$(sweeps)"
