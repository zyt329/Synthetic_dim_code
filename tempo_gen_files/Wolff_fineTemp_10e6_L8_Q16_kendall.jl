...
function driver(;
    ...
    for k = 1:length(J1)
        ...
        simulation["E(T)"][k] = []
        simulation["C(T)"][k] = []=#
        Temp = range(Temps[k][1], Temps[k][2], length = 50)
        Esamples = []
        Csamples = []
        Bsamples = []
...
my_time = Dates.now()
save(
    "/nfs/home/zyt329/Research/Synthetic_dim_code/Periodic_result/Metropolis_Wolff_fineTemp_L_$(L)__Q_$(Q)__sweeps_10e6_Date_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld",
    "sim",
    sim,
)
println("L=$L, Q=$Q, Wolff_fineTemp_10e6 finished at $(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS"))")
#=save(
