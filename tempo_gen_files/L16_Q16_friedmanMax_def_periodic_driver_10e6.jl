...
"""
function driver(; L = 8, Q = 16, J1 = [range(0.0, 1.0, length = 5);0.4;0.45], sweeps = 10^6, Temp = range(0.4, 0.9, length = 100))
    Esim = []
    Csim = []
    Bsim = []
    simulation = []
...
save("/nfs/home/zyt329/Research/Synthetic_dim_code/Periodic_result/Metropolis_Max_def_periodic_L_$(L)__Q_$(Q)__sweeps_10e6_Date_$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld", "sim", sim)
println("L=$L, Q=$Q, Max_def_periodic finished at $(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS"))")
