...
function Ground_state_search(; J = [0.0, 1.0], L = 2, Q = 4)
    ...
    println("For L = $(L), Q = $(Q), J = [$(J[1]), $(J[1])], Lowest E is $(lowest_E_states[1].E), and degeneracy is $(length(lowest_E_states))")
...
my_time = Dates.now()
save(
    "/nfs/home/zyt329/Research/Synthetic_dim_code/Ground_state_search/L$(L)_Q$(Q)_J_[$(J[1]),$(J[1])]_Date$(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS")).jld",
    "lowest_E_states",
    lowest_E_states,
)
println("L=$L, Q=$Q, L$(L)_Q$(Q)_J_[$(J[1]),$(J[1])] finished search at $(Dates.format(my_time, "e_dd_u_yyyy_HH_MM_SS"))")
